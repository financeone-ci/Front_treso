...
function ListUserSite(props) {
  ...
  const handleToggle = (type, tiers) => () => {
    const currentIndex = props.checked.indexOf(type)
    ...
    if (currentIndex === -1) {
      // Ajout de type au tiers
      axios
        .get(`tiers/GestTypeTiersTiers.php?tiers=${tiers}&type=${type}&act=${0}`, {
          headers,
        })
        .then((response) => {
          console.log(type)
      ...
      // Suppression de type du tiers
      axios
        .get(`tiers/GestTypeTiersTiers.php?tiers=${tiers}&type=${type}&act=${1}`, {
          headers,
        })
        .then((response) => {
          if (response.data.reponse == 'error') {
    ...
    <>
      <ModalForm
        ...
        <List dense className={classes.root}>
          {props.typeTiers.map((item) => {
            const labelId = item.id
            console.log(item)
            return (
              <ListItem key={item.id} button>
                <ListItemAvatar>
                  <Avatar alt={item.LIBELLE_TYPE_TIERS} src={'/broken-image.jpg'} />
                </ListItemAvatar>
                <ListItemText
                  id={labelId}
                  primary={`${item.LIBELLE_TYPE_TIERS} `}
                />
