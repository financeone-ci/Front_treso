...
function Autoriser(props) {
  ...
  const DroitsUser = leMenu.group['Paiements'][2]
  ...
  const fetchValidation = async () => {
    const response = await fetch(Constantes.URL + '/autorisations.php?type=R')
    return response.json()
  }
  const { data, status } = useQuery(['autorisations'], fetchValidation)

  // validation des paiements (mise à jour des status des paiements)
  const ValidePaiement = async () => {
    const response = await fetch(
      Constantes.URL + `autorisations.php?type=U&id=${listId}`,
    )
    return response.json()
  }
  ...
  const mutationValidate = useMutation(ValidePaiement, {
    ...
    onSuccess: (data) => {
      setNotify({ message: data.message, type: data.reponse })
      queryClient.invalidateQueries('autorisations')
      setOpens_(false)
      setOpenNotif(true)
    },
  })
  ...
  return (
    <>
      ...
      <ModalOuiNon
        ...
        titre={'Confirmer autorisations !'}
        message={'Voulez vous autoriser ce(s) Paiement(s) ?'}
        non='Annuler'
        oui='Oui'
