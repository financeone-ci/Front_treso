...
function AuditsUsers(props) {
 ...
 today2 = `${yyyy}-${mm}-${dd - 1}`;
  ...
  const matchDate = (values) => {
    axios({
       url: `/auditCnx.php?type=R&debut=${values.debut}&fin=${values.fin}`,
    })
      .then((response) => {
        if (response.data.reponse == 'success') {
          ...
          fetch(Constantes.URL + '/auditCnx.php?type=R&debut='+values.debut+'&fin='+values.fin)
  ...
  useEffect(() => {
    setLoader(true)
    fetch(Constantes.URL + '/auditCnx.php?type=R&debut='+initial.debut+'&fin='+initial.fin)
      .then((response) => response.json())
      .then((data) => {
        setListUsers(data.infos)
      })
  ...
  return (
    <>
      <PageHeader icone={props.icone} titrePage="Audits Connexions" />
