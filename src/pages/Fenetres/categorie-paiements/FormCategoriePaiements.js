...
function FormCategoriePaiements(props) {
  // Stats
  ...
  // Création d'une nouvelle CategoriePaiements
  const submitCategoriePaiements = async (values) => {
    let response = ''
    const headers = {
    ...
    if (values.id === '') {
      response = await axios.post(
        'categorie-paiements/CreateCategoriePaiements.php',
        { values },
        { headers },
      )
    } else {
      response = await axios.post(
        'categorie-paiements/UpdateCategoriePaiements.php?id=' + values.id,
        { values },
        { headers },
      )
    }
  ...
  // Création d'une CategoriePaiements
  const CategoriePaiements = useMutation(submitCategoriePaiements, {
    onSuccess: (data) => {
      console.log(data)
      props.queryClient.invalidateQueries('listecategoriepaiements')
      props.setNotify({
        type: data.reponse,
  ...
  return (
    <>
      <ModalForm
        ...
        {CategoriePaiements.isLoading && <SpinnerForm />}
        <Formik
          noValidate
          initialValues={{
          ...
          onSubmit={(values, onSubmitProps) => {
            CategoriePaiements.mutate(values, {
              onSuccess: (e) => {
                // console.log(e)
                e.reponse == 'success' &&
...
export default FormCategoriePaiements
