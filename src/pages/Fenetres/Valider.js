...
function Valider(props) {
  ...
  const DroitsUser = leMenu.group['Paiements'][1]
  ...
  const fetchValidation = async () => {
    const response = await fetch(Constantes.URL + '/validations.php?type=R')
    return response.json()
  }
  const { data, status } = useQuery(['validations'], fetchValidation)

  // validation des paiements (mise à jour des status des paiements)
  const ValidePaiement = async () => {
    const response = await fetch(
      Constantes.URL + `validations.php?type=U&id=${listId}`,
    )
    return response.json()
  }
  ...
  return (
    <>
      ...
      <ModalOuiNon
        ...
        titre={'Confirmer validation !'}
        message={'Voulez vous valider ce(s) Paiement(s) ?'}
        non='Annuler'
        oui='Oui'
